...
- **`IS_BACKTESTING` env var**: The dashboard auto-sets `IS_BACKTESTING=True` before importing lumibot. Without this, lumibot's `credentials.py` crashes trying to instantiate live brokers. This is already handled in `dashboard.py` legacy/dashboard.py.
