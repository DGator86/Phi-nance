...
/* Headers: purple → orange gradient text or orange accent */
...
/* Expanders: dark purple with orange left border */
...
# --- Tier 2: Compounding (compact, one row + expander) ---
...
# --- Tier 3: Test window (compact) ---
st.subheader("Test")
testing_mode = st.radio(
    "Mode",
    options=[m["id"] for m in TESTING_MODES],
    format_func=lambda x: next((m["name"] for m in TESTING_MODES if m["id"] == x), x),
    horizontal=True,
